...
EOF
	...
	# Station 1.5: Link the custom file to the main bashrc
REAL_BASHRC="$HOME/.bashrc"
HOOK_LINE="source $TARGET"

if ! grep -q "$HOOK_LINE" "$REAL_BASHRC"; then
    echo -e "${BLUE}Linking custom environment to .bashrc...${GREY}"
    echo "$HOOK_LINE" >> "$REAL_BASHRC"
else
    echo -e "${BLUE}Link already exists in .bashrc. Skipping.${GREY}"
fi
